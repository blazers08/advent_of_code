...
func removebyindex(target [][][]int, index int) [][][]int {
	ret := make([][][]int, 0)
	ret = append(ret, target[:index]...)
	return append(ret, target[index+1:]...)
}
	...
	for last_win != true {
		for _, num := range numbers {
			...
			for index < len(tables) {
				if check_win(tables[index]) == true {
					if len(tables) > 1 {
						tables = removebyindex(tables, index)
					} else {
						last_win = true
						fmt.Println(num * sumofunmarked(tables[index]))
						os.Exit(0)
					}
				} else {
					index += 1
